...
C<a-b/N> and comma separated lists. Times are interpreted in UTC. Adding a schedule with an existing name updates it;
reusing the same cron expression preserves the next firing time, changing it recomputes it. Invalid expressions are
