...
The priority is selected when a job is enqueued with L<Minion/"enqueue">. Jobs with a higher priority get performed
first, the default priority is C<0>. Priorities can be positive or negative, but should be in the range between C<100>
and C<-100>.
