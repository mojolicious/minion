...
They are not to be confused with time based job schedulers, such as cron or systemd timers, but L<Minion> has built in
support for cron style L</"Recurring jobs"> when you want a task to run on a wall clock schedule.
...
Jobs can be managed right from the command line with L<Minion::Command::minion::job>, and recurring schedules with
L<Minion::Command::minion::schedule>.
...
For jobs that should run on a wall clock schedule, use L<Minion::Command::minion::schedule> with a cron expression.
Schedules are stored in the database as the source of truth, so they survive restarts and only fire once per interval
no matter how many workers are running.

  # Daily cleanup at 4am
  $ ./myapp.pl minion schedule -e daily_cleanup -c '0 4 * * *' -t cleanup

A cron expression is five fields separated by spaces, one for each part of the wall clock. All times are interpreted
in B<UTC>, not local time.
...
A C<*> means "every value", so C<* * * * *> reads as "every minute of every hour, every day". Pin a field to a
specific number to fix that part of the clock.

  0 4 * * *       # 04:00 every day
  30 9 * * 1      # 09:30 every Monday
  0 0 1 1 *       # midnight on January 1st (once a year)

Use C</N> for "every Nth value", C<a-b> for ranges, and commas for lists.
...
When both C<day-of-month> and C<day-of-week> are restricted, the job fires when B<either> matches, following the
standard Vixie L<cron(8)> behavior. So C<0 0 1 * 0> means "midnight on the 1st of the month B<and> every Sunday at
midnight". Times are interpreted in UTC.

Arguments, priorities, queues and retries can be set per schedule with the same options as
...
Adding a schedule with an existing name updates it; reusing the same cron expression preserves the next firing time,
changing it recomputes it. Invalid expressions are rejected immediately.
...
Schedules can also be declared in code with L<"schedule" in Minion|Minion/"schedule"> and managed from the
L<Mojolicious::Plugin::Minion::Admin> UI.

  $minion->schedule(daily_cleanup => '0 4 * * *' => 'cleanup');

Running workers tick L<"dispatch_schedules" in Minion|Minion/"dispatch_schedules"> every C<dispatch_interval> seconds
(default C<30>) to enqueue any due jobs, coordinated through a Postgres advisory lock so a single dispatch cycle never
fires the same schedule twice. If no worker has been running, the dispatcher will not catch up missed runs; the next
firing time is always the next match strictly after the current time.
...
The C<parents> option of L<"enqueue" in Minion|Minion/"enqueue1"> can be used to make a new job depend on
one or more existing jobs, which all need to have reached the state C<finished> before it can be processed.
...
If a parent job transitions to the C<failed> state, all dependent jobs will remain C<inactive> and wait until
it has been retried successfully or removed. You can use the C<lax> option to allow a job to be processed
even if its parents have C<failed>.

  # Job will be processed even if parent jobs have failed
  $minion->enqueue('cleanup', [], {parents => [$id], lax => 1});

=head2 Automatic retries

The C<attempts> option of L<"enqueue" in Minion|Minion/"enqueue1"> can be used to allow a job to be retried
automatically if it fails, up to the specified number of attempts.

  # Job will be attempted up to five times
  $minion->enqueue('check_links', ['https://mojolicious.org'], {attempts => 5});

The delay between retries is determined by L<Minion/"backoff">, which defaults to C<(retries ** 4) + 15>
(15, 16, 31, 96, 271, 640...) seconds, allowing roughly 25 attempts to be made over 21 days. You can change
the formula by providing your own callback.
...
L<Minion/"lock"> and L<Minion/"unlock"> can be used to limit the concurrency of certain tasks, or to
implement rate limiting for access to external services.
...
The second argument is the number of seconds after which the lock will expire automatically, in case the job
fails to release it. An optional C<limit> argument can be used to allow more than one lock with the same name
...
For convenience you can also use L<Minion/"guard"> to release the lock automatically as soon as the job is
...
=head2 Result promises

L<Minion/"result_p"> can be used to wait for the result of a job asynchronously, without blocking the event
loop. It returns a L<Mojo::Promise> object that will be fulfilled when the job reaches the state C<finished>,
or rejected when it reaches the state C<failed>.
...
=head2 Foreground debugging

L<Minion/"foreground"> can be used to retry a job in the C<minion_foreground> queue and perform it right
away in the current process, making it very easy to debug a failed job with the full output of your
application available. This is usually done from the command line with L<Minion::Command::minion::job>.

  $ ./myapp.pl minion job -f 10023

=head2 Worker signals

Worker processes can be controlled at runtime with signals. C<INT> and C<TERM> will stop the worker
gracefully after finishing its current jobs, while C<QUIT> will stop it immediately.

  # Stop worker gracefully
  $ kill -INT $worker_pid

Job processes spawned by the worker can also receive signals. C<INT> and C<TERM> start out with the
operating system default, allowing jobs to install custom signal handlers to stop gracefully. C<USR1> and
C<USR2> start out being ignored, and are available for jobs to use as they see fit.
...
L<Minion/"broadcast"> can be used to send remote control commands to one or more workers. Several commands
are built in, such as C<stop> to kill a job, C<kill> to send a signal to a job process, and C<jobs> to
change how many jobs a worker runs concurrently. This is usually done from the command line with
L<Minion::Command::minion::job>.
...
You can also add your own commands to workers with L<Minion::Worker/"add_command">.
...
L<Minion/"perform_jobs"> can be used to perform all jobs in the queue with a temporary worker, making it
very easy to test background tasks without having to start a real worker process.
...
L<Minion/"jobs"> can be used to safely iterate through job information, with a variety of filters to narrow
down the results.
...
Named locks can also be used to limit how often a job accesses an external service, by letting them expire
naturally instead of releasing them manually with L<Minion/"unlock">.
...
Each lock acquired this way will count against the limit for the duration of its expiration time, effectively
capping throughput to C<limit> jobs per expiration period.
