...
The priority is selected when a job is enqueued with L<"enqueue" in Minion|Minion/"enqueue1">. Jobs with a higher
priority get performed first, the default priority is C<0>. Priorities can be positive or negative, but should be in the
range between C<100> and C<-100>.
...
To raise or lower the priority of a job you can use L<Minion::Job/"retry">.
