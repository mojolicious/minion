...
For jobs that should run on a wall clock schedule, use L<"schedule" in Minion|Minion/"schedule"> with a standard five
field cron expression. Schedules are stored in the database as the source of truth, so they survive restarts and only
fire once per interval no matter how many workers are running.

  # Daily cleanup at 4am, no arguments
  $minion->schedule(daily_cleanup => '0 4 * * *' => 'cleanup');

  # Every five minutes, with arguments and options
  $minion->schedule(
    refresh => '*/5 * * * *' => 'refresh_cache' => ['users'] => {priority => 5});

  # Weekdays at 9, with retries
  $minion->schedule(
    weekday_report => '0 9 * * 1-5' => 'report' => [] => {attempts => 3});

The expression uses the standard fields C<minute hour day-of-month month day-of-week>, with C<*>, C<*/N>, C<a-b>,
C<a-b/N> and comma separated lists. Times are interpreted in the local timezone, matching Unix L<cron(8)>. Calling
L<"schedule" in Minion|Minion/"schedule"> again with the same name updates the schedule; using the same cron expression
preserves the next firing time, changing it recomputes it. Invalid expressions raise an exception immediately.
...
firing time is always the next match strictly after the current time.
  ...
  $minion->pause_schedule('daily_cleanup');
  $minion->resume_schedule('daily_cleanup');

  # Inspect the firing time of a schedule
  my $epoch = $minion->list_schedules(0, 1, {names => ['daily_cleanup']})->{schedules}[0]{next_run};

  # Remove a schedule
  $minion->unschedule('daily_cleanup');

Schedules are also visible in the L<Mojolicious::Plugin::Minion::Admin> UI, where they can be paused, resumed and
removed, and can be managed from the command line with L<Minion::Command::minion::schedule>.
