...
sub _try {
    ...
    return $self->pg->db->query(
        q{SET LOCAL lock_timeout TO '50ms';
        WITH cte AS (
        ...
        RETURNING id, args, retries, task}, $options->{id}, $options->{min_priority},
        $options->{queues} || ['default'], [keys %{$self->minion->tasks}], $id
    )->expand->hash;
}
