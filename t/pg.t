...
### my $workers = $minion->backend->_workers;
...
my $doc; ###
my $info = $minion->backend->_workers->{$id};
ok $info, 'is registered';
my $pid = 4000;
$pid++ while kill 0, $pid;
$minion->backend->pg->db->query("UPDATE worker SET pid = ? WHERE id = ?", $pid, $id);
$minion->repair;
...
$minion->backend->pg->db->query("UPDATE job SET finished = to_timestamp(?) WHERE id = ?", $t->{finished} - 864001, $id2);
$t = $minion->backend->pg->db->query("SELECT EXTRACT(EPOCH FROM finished) as finished FROM job WHERE id = ?", $id3)->hash;
$minion->backend->pg->db->query("UPDATE job SET finished = to_timestamp(?) WHERE id = ?", $t->{finished} - 864001, $id3);
$worker->unregister;
$minion->repair;
...
# Reset
$minion->reset->repair;
my $ret = $minion->backend->pg->db->query("SELECT count(id) as count FROM job")->hash;
ok !$ret->{count},    'no jobs';
$ret = $minion->backend->pg->db->query("SELECT count(id) as count FROM worker")->hash;
ok !$ret->{count}, 'no workers';
...
$info = $minion->job($id)->info;
ok int($info->{delayed}) > time, 'delayed timestamp';
$minion->backend->pg->db->query("UPDATE job SET delayed = ? WHERE id = ?", time - 100, $id);
$job = $worker->dequeue(0);
is $job->id, $id, 'right id';
